...
void saveLog(const std::string& message) {
    ...
    std::string filePath = "/storage/emulated/0/Android/data/com.catsbit.oxidesurvivalisland/files/log.txt";
      
    std::time_t now = std::time(nullptr);
    std::tm* current_time = std::localtime(&now);
    char buffer[80];
    std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", current_time);
    std::string logMessage = "[" + std::string(buffer) + "] " + message + "\n";
  
    logFile.open(filePath, std::ios_base::app);
    if (logFile.is_open()) {
        logFile << logMessage;
        logFile.close();
        std::cout << "Лог сохранен успешно." << std::endl;
    } else {
        std::cout << "Ошибка при сохранении лога." << std::endl;
    }
}
// =========================================================================== //
...
// ================================================================================================================ //
void *Il2CppGetMethodInfo(const char *image, const char *namespaze, const char *clazz, const char *name, int argsCount) {
    void *img = Il2CppGetImageByName(image);
    if(!img) return 0;
    void *klass = Il2CppGetClassType(image, namespaze, clazz);
    if(!klass) return 0;
...
void *Il2cppRuntimeInvoke(void *method, void *obj, void **params, void **exc) {
    if (!il2cpp_runtime_invoke) return 0;
