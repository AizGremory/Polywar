...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>POLYWAR CHEAT v2.7</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:red;\">KUBOOM / POLYWAR | by AizGremory</p>"
                                     "</marquee></b>"));

    //Toast Example
    Toast(env,ctx,OBFUSCATE("- POLYWAR CHEAT LOADED -"),ToastLength::LENGTH_LONG);
